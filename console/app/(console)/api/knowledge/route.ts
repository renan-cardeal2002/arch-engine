...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      throw new Error(data?.detail || "Erro ao criar a conhecimento");
    }

    return NextResponse.json(data);
  } catch (error) {
    console.error("Erro na rota /knowledge:", error);
...
export async function GET(_request: NextRequest) {
  try {
    ...
    try {
      ...
      throw new Error(`Erro ao buscar conhecimentos: ${text}`);
    }

    if (!response.ok) {
      console.error("Erro da API:", data);
      throw new Error(
        data?.detail || `Erro ${response.status} ao buscar conhecimentos`
      );
    }
