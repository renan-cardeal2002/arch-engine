...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/tool/${body.id}/endpoints`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(
        data?.detail || "Erro ao criar a endpoint para a ferramenta"
      );
    }
...
export async function GET(
  ...
  try {
    const token = await _get_token();
    const response = await fetch(`${API_URL}/tool/${params.id}/endpoints`, {
      method: "GET",
      headers: {
